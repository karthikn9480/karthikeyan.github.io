...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // Add click-to-expand functionality for mobile devices
    certBoxes.forEach(box => {
        box.addEventListener('click', function(e) {
            if (window.innerWidth <= 768) {
                ...
                // Ensure details are visible on mobile
                const certInfo = this.querySelector('.cert-info');
                if (this.classList.contains('expanded')) {
                    certInfo.style.display = 'flex';
                    certInfo.style.opacity = '1';
                } else {
                    certInfo.style.opacity = '0.7';
                }
            }
        });
    });
